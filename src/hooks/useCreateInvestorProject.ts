...
export function useCreateInvestorProject() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!Number.isFinite(montant) || montant <= 0) {
      return setError("يرجى إدخال مبلغ استثمار صحيح وأكبر من صفر.");
    }
    if (!Number.isFinite(emplois) || emplois < 0) {
      return setError("يرجى إدخال عدد مناصب عمل صحيح.");
    }
    if (form.superficie.trim() && (!Number.isFinite(superficie) || Number(superficie) < 0)) {
      return setError("يرجى إدخال مساحة صحيحة.");
    }
    ...
    if (form.date_debut && form.date_fin) {
      ...
      if (Number.isFinite(start) && Number.isFinite(end) && end < start) {
        return setError("تاريخ نهاية المشروع يجب أن يكون بعد تاريخ البداية.");
      }
    }

    if (sectorId !== null && (!Number.isFinite(sectorId) || sectorId <= 0)) {
      return setError("القطاع الاستثماري غير صالح.");
    }
    ...
    try {
      const response = await fetch(CREATE_PROJECT_API_URL, {
        ...
        body: JSON.stringify({
          ...
          montant_investissement: montant,
          nombre_emplois: emplois,
          superficie: superficie !== null && Number.isFinite(superficie) ? superficie : null,
          unite_superficie: form.unite_superficie || "m²",
          sector_id: sectorId,
      ...
      if (!contentType.toLowerCase().includes("application/json")) {
        throw new Error("Le serveur a retourné une réponse invalide. Vérifiez create-project.php.");
      }

      const data = await response.json();
      if (!response.ok || !data?.success) {
        throw new Error(data?.message || "تعذر إنشاء المشروع.");
      }
