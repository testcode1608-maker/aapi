...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<I18nContextValue>(() => ({ language, setLanguage, t: (key: string) => { const parts = key.split("."); const current = resolveKey(translations[language], parts); if (typeof current === "string") return current; const home = resolveKey(homeTranslations[language], parts); if (typeof home === "string") return home; const fallback = resolveKey(translations.ar, parts); if (typeof fallback === "string") return fallback; const homeFallback = resolveKey(homeTranslations.ar, parts); return typeof homeFallback === "string" ? homeFallback : key; } }), [language]);
