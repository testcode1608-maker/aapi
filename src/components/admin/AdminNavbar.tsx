...
export default function AdminNavbar({ onToggle }: { onToggle: () => void }) {
  ...
  const items: Array<[string, string, ReactNode]> = [
    ...
    ["/admin/announcements", "الإعلانات", <MessageSquare size={18} />],
  ];
  ...
  return <>
    ...
    <aside className="admin-navbar" dir={direction} lang={language} style={{ left: direction === "ltr" ? 0 : "auto", right: direction === "rtl" ? 0 : "auto" }}>
