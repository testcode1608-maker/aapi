    ...
    <section className="investor-dashboard-section">
      ...
      <div className="investor-dashboard-grid">
        <div className="investor-dashboard-card investor-dashboard-projects-card">
          <div className="investor-dashboard-card-header"><div><span className="investor-dashboard-card-overline">المشاريع</span><h2>آخر مشاريعي</h2></div><a href="/investor/dashboard/projects">عرض الكل</a></div>
          <div className="investor-dashboard-project-list">
            {latestProjects.map((project) => { const status = getProjectStatus(project.statut); return (
              <div className="investor-dashboard-project-item" key={project.id}>
                ...
                <div className="investor-dashboard-project-info"><strong>{project.titre}</strong><span>{project.secteurs || "قطاع استثماري"}</span><small>{formatAmount(project.montant_investissement)} · {project.nombre_emplois} منصب</small></div>
        ...
        <div className="investor-dashboard-card investor-dashboard-investment-summary">
          <div className="investor-dashboard-card-header"><div><span className="investor-dashboard-card-overline">الاستثمارات</span><h2>ملخص الاستثمارات</h2></div></div>
          <div className="investor-dashboard-investment-total"><span>إجمالي الاستثمارات</span><strong>{formatAmount(stats?.total_investment)}</strong></div>
          <div className="investor-dashboard-progress"><div className="investor-dashboard-progress-header"><span>نسبة المشاريع المكتملة</span><strong>{projectCompletion}%</strong></div><div className="investor-dashboard-progress-bar"><span style={{ width: `${projectCompletion}%` }} /></div></div>
          <div className="investor-dashboard-investment-items"><div><span>الاستثمارات النشطة</span><strong>{toNumber(stats?.investments_active)}</strong></div><div><span>الاستثمارات المكتملة</span><strong>{completedInvestments}</strong></div><div><span>إجمالي العمليات</span><strong>{toNumber(stats?.investments_total ?? investments.length)}</strong></div></div>
        </div>
