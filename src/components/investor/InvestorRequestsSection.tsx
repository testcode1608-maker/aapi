...
export default function InvestorRequestsSection({ requests }: Props) {
  ...
  return (
    <section className="investor-dashboard-section">
      <div className="investor-dashboard-page-header"><div><span className="investor-dashboard-overline">{tr("overline")}</span><h1>{tr("title")}</h1><p>{tr("description")}</p></div><button type="button" className="investor-dashboard-primary-btn" onClick={() => setShowForm(v => !v)}><i className={showForm ? "bi bi-dash-circle" : "bi bi-plus-circle"} /> {showForm ? tr("hideForm") : tr("newRequest")}</button></div>
      ...
      <div className="investor-dashboard-grid">
        {requests.map(request => {
          ...
          return <div className="investor-dashboard-card investor-dashboard-projects-card" key={request.id}><div className="investor-dashboard-card-header"><div><span className="investor-dashboard-card-overline">{request.type_demande || tr("investmentRequest")}</span><h2>{request.objet}</h2></div><span className={status.className}>{status.label}</span></div><div className="investor-dashboard-project-list"><div className="investor-dashboard-project-item"><div className="investor-dashboard-project-icon"><i className="bi bi-file-earmark-text" /></div><div className="investor-dashboard-project-info"><strong>{request.projet_titre || tr("noProject")}</strong><span>{request.wilaya || tr("wilayaUnknown")}</span><small>{request.created_at ? formatDate(request.created_at) : "—"}</small></div></div>{request.description && <p className="investor-request-description">{request.description}</p>}{request.reponse && <div className="investor-request-response"><strong>{tr("adminResponse")}</strong><span>{request.reponse}</span></div>}</div></div>;
        })}
