...
export default function InvestorRequestCreateForm({ form, setForm, setDocuments, projects, creating, error, success, onSubmit, onReset }: Props) {
  ...
  return <section className="investor-dashboard-card investor-request-create-card">
    ...
    <form onSubmit={onSubmit} className="investor-project-form-grid">
      ...
      <div className="investor-project-form-actions investor-project-form-field-full"><button className="investor-dashboard-primary-btn" type="submit" disabled={creating}>{creating ? tr("sending") : tr("send")}</button><button className="investor-dashboard-secondary-btn" type="button" onClick={onReset} disabled={creating}>{tr("clear")}</button></div>
    </form>
  </section>;
}
