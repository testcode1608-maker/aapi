...
function OpportunitiesPage(){
 ...
 useEffect(()=>{fetch(API).then(r=>r.json()).then(j=>{if(j.success)setOpportunities(j.opportunities??[])}).catch(()=>setOpportunities([])).finally(()=>setLoading(false))},[]);
 ...
 return <>
  ...
  <section className="opportunities-content"><div className="container">
   ...
   {loading?<div className="opportunities-empty"><i className="bi bi-hourglass-split"/><h3>{t("opportunitiesPage.results")}</h3></div>:<div className="row g-4">{filtered.map(o=><div className="col-lg-4 col-md-6" key={o.id}><article className="opportunity-card">
